...
In [Part 1]("/posts/deploying-elixir-on-ecs-part-1") we used terraform to build all of the required ECS infrastructure in AWS. Next we'll build an image, push it to the image repo and tell ECS to run it.
...
## Build Configuraton
I like to create a `Makefile` for building my Docker images and pushing them to ECR. Note the `your_ecr_url` is the url of your ECR that was created in [Part 1]("/posts/deploying-elixir-on-ecs-part-1").
