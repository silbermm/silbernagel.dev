defprotocol Silbernageldev.OpenTelemetry.Encoder do
  @moduledoc """
  ...
  must ensure the `Silbernageldev.OpenTelemetry.Encoder.encode/1` function returns a value with a type the
...
defimpl Silbernageldev.OpenTelemetry.Encoder, for: Any do
  def encode(_term) do
    :otel_no_encoder
  end
end

defimpl Silbernageldev.OpenTelemetry.Encoder, for: [PID, Port, Reference, Function] do
  def encode(term) do
    inspect(term)
  end
end

defimpl Silbernageldev.OpenTelemetry.Encoder, for: [Integer, Float, BitString, Atom] do
  def encode(term) do
    term
  end
end

defimpl Silbernageldev.OpenTelemetry.Encoder, for: Map do
  def encode(term) do
    Enum.into(term, Map.new(), fn {key, value} ->
      {key, Silbernageldev.OpenTelemetry.Encoder.encode(value)}
    end)
  end
end

defimpl Silbernageldev.OpenTelemetry.Encoder, for: Tuple do
  def encode(term) do
    term
    |> Tuple.to_list()
    |> Silbernageldev.OpenTelemetry.Encoder.encode()
  end
end

defimpl Silbernageldev.OpenTelemetry.Encoder, for: List do
  def encode(term) do
  ...
  defp encode_proper_list(term) do
    encoded = for item <- term, do: Silbernageldev.OpenTelemetry.Encoder.encode(item)
