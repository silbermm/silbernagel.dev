defmodule Silbernageldev.OpenTelemetry do
  @moduledoc """
  ...
  can define custom spans using the `Silbernageldev.OpenTelemetry.trace/1` macro:
  ...
  You can pass the following options to `use Silbernageldev.OpenTelemetry` to configure the calling module:

  * `debug` - should debug statements be printed to stdout during compilation? Defaults to `false`.
  """

  alias Silbernageldev.OpenTelemetry.Encoder
  ...
  defmacro __using__(opts \\ []) do
    ...
    quote do
      import Silbernageldev.OpenTelemetry
      ...
      Module.put_attribute(__MODULE__, :on_definition, Silbernageldev.OpenTelemetry)
      Module.put_attribute(__MODULE__, :before_compile, Silbernageldev.OpenTelemetry)
  ...
  defmacro __before_compile__(env) do
    ...
    unless Enum.any?(function_traces) do
      warn!(line, env.file, """
      use Silbernageldev.OpenTelemetry called but no traces are defined, are you missing trace/1 or trace_all/1?
      """)
    end
        ...
        quote do
          unquote(kind)(unquote(name)(unquote_splicing(positional_args))) do
            OpenTelemetry.Tracer.with_span unquote(span_name),
              ...
              try do
                Silbernageldev.OpenTelemetry.__record_threads__()

                Silbernageldev.OpenTelemetry.__record_arguments__(
                  unquote(positional_args),
                  unquote(record_arguments)
                )

                result = super(unquote_splicing(positional_args))
                Silbernageldev.OpenTelemetry.__record_results__(result, unquote(record_results))
                result
              rescue
                exception ->
  ...
  * `thread.name` - the `PID` of the process executing the annotated function.
  ...
  defmacro trace(opts \\ []) when is_list(opts) do
    quote bind_quoted: [opts: opts] do
      Silbernageldev.OpenTelemetry.__trace__!(__MODULE__, __ENV__.line, __ENV__.file, opts)
    end
  end
  ...
  defmacro trace_all(opts \\ []) when is_list(opts) do
    quote bind_quoted: [opts: opts] do
      Silbernageldev.OpenTelemetry.__trace_all__!(__MODULE__, __ENV__.line, __ENV__.file, opts)
    end
  end
